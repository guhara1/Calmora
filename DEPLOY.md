...
- 루트(`/`)는 `/incheon/` 로 리다이렉트됩니다(`_redirects`).
...
2. `www.calmora.co.kr`(및 `calmora.co.kr`) 추가 → 안내되는 CNAME/A 레코드를 DNS에 등록
3. 도메인이 활성화되면 canonical·og:image·sitemap(이미 `https://www.calmora.co.kr` 기준)과
   완전히 일치합니다.
...
- Google Search Console에 도메인 등록 → `https://www.calmora.co.kr/sitemap.xml` 제출
- `robots.txt` 는 2026 개편 준비 페이지(`/incheon/jemulpo-gu/` 등)를 차단하도록 이미 설정됨
